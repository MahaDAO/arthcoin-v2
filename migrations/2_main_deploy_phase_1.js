...
module.exports = async function (deployer, network, accounts) {
  ...
  if (process.env.MIGRATION_MODE == 'ganache') {
    ...
    send.ether(COLLATERAL_ARTH_AND_ARTHS_OWNER, METAMASK_ADDRESS, 2e18);
  }
  ...
  await deployer.link(Math, [StakingRewards_ARTH_WETH, StakingRewards_ARTH_WETH, StakingRewards_ARTH_USDC, StakingRewards_ARTH_ARTHS, StakingRewards_ARTHS_WETH, UniswapV2ERC20, UniswapV2Pair]);
  await deployer.deploy(SafeMath);
  await deployer.link(SafeMath, [ERC20, ERC20Custom, SafeERC20, WETH, FakeCollateral_USDC, FakeCollateral_USDT, ARTHStablecoin, Pool_USDC, Pool_USDT, ARTHShares, StakingRewards_ARTH_WETH, StakingRewards_ARTH_USDC, StakingRewards_ARTH_ARTHS, StakingRewards_ARTHS_WETH, UniswapV2ERC20, UniswapV2Library, UniswapV2Router02, UniswapV2Router02_Modified, SwapToPrice, Timelock]);
  await deployer.deploy(TransferHelper);
  await deployer.link(TransferHelper, [UniswapV2Router02, UniswapV2Router02_Modified, SwapToPrice, StakingRewards_ARTH_WETH, StakingRewards_ARTH_USDC, StakingRewards_ARTH_ARTHS, StakingRewards_ARTHS_WETH, Pool_USDC, Pool_USDT]);
  ...
  await deployer.link(UniswapV2Library, [UniswapPairOracle_ARTH_WETH, UniswapPairOracle_ARTH_USDC, UniswapPairOracle_ARTHS_WETH, UniswapPairOracle_ARTHS_USDC, UniswapPairOracle_USDC_WETH, UniswapV2Router02, UniswapV2Router02_Modified, SwapToPrice]);
  ...
  await deployer.link(SafeERC20, [WETH, FakeCollateral_USDC, FakeCollateral_USDT, ARTHStablecoin, Pool_USDC, Pool_USDT, ARTHShares, StakingRewards_ARTH_WETH, StakingRewards_ARTH_USDC, StakingRewards_ARTH_ARTHS, StakingRewards_ARTHS_WETH]);
  ...
  // ARTH
  await deployer.deploy(ARTHStablecoin, "Arth", "ARTH", COLLATERAL_ARTH_AND_ARTHS_OWNER, timelockInstance.address);
  const arthInstance = await ARTHStablecoin.deployed();

  // ARTHS
  await deployer.deploy(ARTHShares, "Arth Share", "ARTHS", ORACLE_ADDRESS, COLLATERAL_ARTH_AND_ARTHS_OWNER, timelockInstance.address);
  ...
  console.log(`arths_name: [${arths_name}]`);
  ...
  // ======== Set ARTH ARTHS address ========
  console.log(chalk.yellow('===== ARTH ARTHS ADDRESS ====='));

  // Link the ARTHS contract to the ARTH contract
  await arthInstance.setARTHSAddress(arthsInstance.address, { from: COLLATERAL_ARTH_AND_ARTHS_OWNER });
  ...
  if (IS_MAINNET) {
    ...
    await deployer.deploy(WETH, COLLATERAL_ARTH_AND_ARTHS_OWNER);
    await deployer.deploy(FakeCollateral_USDC, COLLATERAL_ARTH_AND_ARTHS_OWNER, ONE_HUNDRED_MILLION_DEC6, "USDC", 6);
    await deployer.deploy(FakeCollateral_USDT, COLLATERAL_ARTH_AND_ARTHS_OWNER, ONE_HUNDRED_MILLION_DEC6, "USDT", 6);
