...
module.exports = async function (deployer, network, accounts) {
  ...
  if (IS_MAINNET) {
    await Promise.all([
      await col_instance_USDC.transfer(pool_instance_USDC.address, ONE_HUNDRED_DEC6, { from: COLLATERAL_ARTH_AND_FXS_OWNER }),
      await col_instance_USDT.transfer(pool_instance_USDT.address, ONE_HUNDRED_DEC6, { from: COLLATERAL_ARTH_AND_FXS_OWNER }),
    ]);
  }
  else {
    console.log(chalk.blue('=== USDC ==='));
    await col_instance_USDC.transfer(pool_instance_USDC.address, COLLATERAL_SEED_DEC6, { from: COLLATERAL_ARTH_AND_FXS_OWNER });

    console.log(chalk.blue('=== USDT ==='));
    await col_instance_USDT.transfer(pool_instance_USDT.address, COLLATERAL_SEED_DEC6, { from: COLLATERAL_ARTH_AND_FXS_OWNER });
