...
describe('Staking Reward', () => {
  ...
  describe('- Access restricted functions', async () => {
    it(' - Should not work if not (owner || governance)', async () => {
      await expect(
        boostedStaking.connect(whale).setArthController(owner.address)
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking.connect(whale).setRewardsDuration(ETH)
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking.connect(whale).setMultipliers(ETH, ETH)
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking
          .connect(whale)
          .setLockedStakeTimeForMinAndMaxMultiplier(ETH, ETH)
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking.connect(whale).initializeDefault()
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking.connect(whale).greylistAddress(owner.address)
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking.connect(whale).unlockStakes()
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking.connect(whale).setRewardRate(1)
      ).to.revertedWith('You are not the owner or the governance timelock');

      await expect(
        boostedStaking
          .connect(whale)
          .setOwnerAndTimelock(whale.address, whale.address)
      ).to.revertedWith('You are not the owner or the governance timelock');
    });

    it(' - Should work if (owner || governance)', async () => {
      await expect(
        boostedStaking.connect(owner).setArthController(owner.address)
      ).to.not.reverted;
      await expect(
        boostedStaking.connect(timelock).setArthController(owner.address)
      ).to.not.reverted;

      await advanceTimeAndBlock(provider, 7 * 24 * 60 * 61);
      boostedStaking.connect(owner).setRewardsDuration(10);
      expect(await boostedStaking.rewardsDuration()).to.eq(10);
      boostedStaking.connect(timelock).setRewardsDuration(7);
      expect(await boostedStaking.rewardsDuration()).to.eq(7);

      await boostedStaking.connect(owner).setMultipliers(ETH, ETH);
      expect(await boostedStaking.lockedStakeMaxMultiplier()).to.eq(ETH);
      expect(await boostedStaking.crBoostMaxMultiplier()).to.eq(ETH);
      await boostedStaking.connect(timelock).setMultipliers(1, 1);
      expect(await boostedStaking.lockedStakeMaxMultiplier()).to.eq(1);
      expect(await boostedStaking.crBoostMaxMultiplier()).to.eq(1);
      ...
      await expect(boostedStaking.connect(owner).initializeDefault()).to.emit(
        boostedStaking,
        'DefaultInitialization'
      );
      await expect(
        boostedStaking.connect(timelock).initializeDefault()
      ).to.emit(boostedStaking, 'DefaultInitialization');

      await boostedStaking.connect(owner).greylistAddress(owner.address);
      expect(await boostedStaking.greylist(owner.address)).to.eq(true);
      await boostedStaking.connect(timelock).greylistAddress(owner.address);
      expect(await boostedStaking.greylist(owner.address)).to.eq(false);

      await boostedStaking.connect(owner).unlockStakes();
      expect(await boostedStaking.isLockedStakes()).to.eq(true);
      await boostedStaking.connect(timelock).unlockStakes();
      expect(await boostedStaking.isLockedStakes()).to.eq(false);

      await boostedStaking.connect(owner).setRewardRate(1);
      expect(await boostedStaking.rewardRate()).to.eq(1);
      await boostedStaking.connect(timelock).setRewardRate(10);
      expect(await boostedStaking.rewardRate()).to.eq(10);

      await boostedStaking
        .connect(owner)
        .setOwnerAndTimelock(whale.address, whale2.address);
      expect(await boostedStaking.ownerAddress()).to.eq(whale.address);
      expect(await boostedStaking.timelockAddress()).to.eq(whale2.address);
      await boostedStaking
        .connect(whale2)
        .setOwnerAndTimelock(owner.address, timelock.address);
      expect(await boostedStaking.ownerAddress()).to.eq(owner.address);
      expect(await boostedStaking.timelockAddress()).to.eq(timelock.address);
    });
  });
  ...
  describe('- Stake', async () => {
    ...
    it(' - Should work for 1 account', async () => {
      ...
      const contractARTHBalanceBefore = await arth.balanceOf(
        boostedStaking.address
      );

      await expect(boostedStaking.stake(ETH))
        .to.emit(boostedStaking, 'Staked')
        .withArgs(owner.address, ETH);

      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH);

      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(ETH);
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.add(ETH));
    ...
    it(' - Should work for 2 accounts with same amounts', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(whale.address)).to.eq(0);
      ...
      expect(await arth.balanceOf(whale.address)).to.eq(
        whaleARTHBalanceBefore.sub(ETH)
      );
      expect(await boostedStaking.totalSupply()).to.eq(ETH.add(ETH));
      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(whale.address)).to.eq(ETH);
    });
    ...
    it(' - Should work for 2 accounts with different amounts', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.add(ETH).add(ETH));
      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(whale.address)).to.eq(
        ETH.add(ETH)
      );
    });
    ...
    it(' - Should fail for grey listed addresses', async () => {
      ...
      await expect(boostedStaking.stake(ETH)).to.revertedWith(
        'address has been greylisted'
      );

      await boostedStaking.greylistAddress(whale.address);
      await expect(boostedStaking.connect(whale).stake(ETH)).to.revertedWith(
        'address has been greylisted'
      );
    });
  });
  ...
  describe('- Stake for', async () => {
    ...
    it(' - Should fail if called by non pool address', async () => {
      await expect(
        boostedStaking
          .connect(whale)
          .stakeFor(whale2.address, whale2.address, ETH)
      ).to.revertedWith('Staking: FORBIDDEN');
      await expect(
        boostedStaking
          .connect(whale)
          .stakeFor(whale2.address, whale.address, ETH)
      ).to.revertedWith('Staking: FORBIDDEN');
      await expect(
        boostedStaking
          .connect(whale)
          .stakeFor(whale2.address, owner.address, ETH)
      ).to.revertedWith('Staking: FORBIDDEN');
    ...
    it(' - Should fail for grey listed addresses', async () => {
      await boostedStaking.greylistAddress(whale.address);
      await expect(
        boostedStaking.stakeFor(whale.address, whale.address, ETH)
      ).to.revertedWith('address has been greylisted');
      await expect(
        boostedStaking.stakeFor(whale.address, owner.address, ETH)
      ).to.revertedWith('address has been greylisted');
      await expect(
        boostedStaking.stakeFor(whale.address, whale2.address, ETH)
      ).to.revertedWith('address has been greylisted');

      await boostedStaking.greylistAddress(whale2.address);
      await expect(
        boostedStaking.stakeFor(whale2.address, whale.address, ETH)
      ).to.revertedWith('address has been greylisted');
      await expect(
        boostedStaking.stakeFor(whale2.address, whale2.address, ETH)
      ).to.revertedWith('address has been greylisted');
      await expect(
        boostedStaking.stakeFor(whale2.address, owner.address, ETH)
      ).to.revertedWith('address has been greylisted');
    });
    ...
    it(' - Should work for 1 account, where staker is spender', async () => {
      ...
      await expect(
        boostedStaking
          .connect(owner)
          .stakeFor(whale.address, whale.address, ETH)
      )
        .to.emit(boostedStaking, 'Staked')
        .withArgs(whale.address, ETH);
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(0);
    });
    ...
    it(' - Should work for 1 account, where another user is spender', async () => {
      ...
      await expect(
        boostedStaking
          .connect(owner)
          .stakeFor(whale.address, whale2.address, ETH)
      )
        .to.emit(boostedStaking, 'Staked')
        .withArgs(whale.address, ETH);
      ...
      expect(await arth.balanceOf(whale.address)).to.eq(whaleARTHBalanceBefore);
      expect(await arth.balanceOf(whale2.address)).to.eq(
      ...
      expect(await boostedStaking.unlockedBalanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(0);
      expect(await boostedStaking.unlockedBalanceOf(whale2.address)).to.eq(0);
    });
    ...
    it(' - Should work for 2 accounts with same amounts, where stakers are spenders', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.mul(2));
      expect(await boostedStaking.unlockedBalanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(whale2.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(0);
    });
    ...
    it(' - Should work for 2 accounts with same amounts, where other users are spenders', async () => {
      ...
      await expect(
        boostedStaking
          .connect(owner)
          .stakeFor(whale.address, owner.address, ETH)
      )
        .to.emit(boostedStaking, 'Staked')
        .withArgs(whale.address, ETH);
      await expect(
    ...
    it(' - Should work for 2 accounts with different amounts where stakers are spenders', async () => {
      ...
      await expect(
        boostedStaking
          .connect(owner)
          .stakeFor(whale2.address, whale2.address, ETH.mul(2))
      )
        .to.emit(boostedStaking, 'Staked')
      ...
      expect(await arth.balanceOf(owner.address)).to.eq(ownerARTHBalanceBefore);
      expect(await boostedStaking.totalSupply()).to.eq(ETH.mul(3));
      expect(await boostedStaking.unlockedBalanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.unlockedBalanceOf(whale2.address)).to.eq(
        ETH.mul(2)
      );
      expect(await boostedStaking.unlockedBalanceOf(owner.address)).to.eq(0);
    });
    ...
    it(' - Should work for 2 accounts with different amounts where other users are spenders', async () => {
      ...
      await expect(
        boostedStaking
          .connect(owner)
          .stakeFor(whale2.address, whale.address, ETH.mul(2))
      )
        .to.emit(boostedStaking, 'Staked')
        .withArgs(whale2.address, ETH.mul(2));
  ...
  describe('- Stake locked', async () => {
    ...
    it(' - Should fail for grey listed addresses', async () => {
      await boostedStaking.greylistAddress(owner.address);
      await expect(boostedStaking.stakeLocked(ETH, 41472000)).to.revertedWith(
        'address has been greylisted'
      );

      await boostedStaking.greylistAddress(whale.address);
      await expect(
        boostedStaking.connect(whale).stakeLocked(ETH, 41472000)
      ).to.revertedWith('address has been greylisted');
    });

    it(' - Should not work lockTime = 0', async () => {
      await expect(boostedStaking.stakeLocked(ETH, 0)).to.revertedWith(
    ...
    it(' - Should not work for lockTime < 7 days', async () => {
      ...
      await expect(boostedStaking.stakeLocked(ETH, 604700)).to.revertedWith(
    ...
    it(' - Should not work for lockTime > 3Y', async () => {
      await arth.connect(owner).approve(boostedStaking.address, ETH);
      await expect(boostedStaking.stakeLocked(ETH, 94608001)).to.revertedWith(
    ...
    it(' - Should work for 1 account', async () => {
      ...
      await expect(boostedStaking.stakeLocked(ETH, 41472000))
        .to.emit(boostedStaking, 'StakeLocked')
      ...
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(ETH);
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.add(ETH));
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(
        ETH.add(ETH)
      );
    });
    ...
    it(' - Should work for 2 accounts with same amount', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(whale.address)).to.eq(0);
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.add(ETH));
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(whale.address)).to.eq(ETH);
    });
    ...
    it(' - Should work for 2 accounts with different amounts', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.add(ETH).add(ETH));
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(whale.address)).to.eq(
        ETH.add(ETH)
      );
    });
  });
  ...
  describe('- Stake locked for', async () => {
    ...
    it(' - Should fail if called by non pool address', async () => {
      await expect(
        boostedStaking
          .connect(whale)
          .stakeLockedFor(whale.address, whale.address, ETH, 41472000)
      ).to.revertedWith('Staking: FORBIDDEN');
      await expect(
        boostedStaking
          .connect(whale)
          .stakeLockedFor(whale.address, whale2.address, ETH, 41472000)
      ).to.revertedWith('Staking: FORBIDDEN');
      await expect(
        boostedStaking
          .connect(whale)
          .stakeLockedFor(whale.address, owner.address, ETH, 41472000)
      ).to.revertedWith('Staking: FORBIDDEN');
      ...
      await expect(
        boostedStaking
          .connect(whale2)
          .stakeLockedFor(whale2.address, whale.address, ETH, 41472000)
      ).to.revertedWith('Staking: FORBIDDEN');
      await expect(
        boostedStaking
          .connect(whale2)
          .stakeLockedFor(whale2.address, whale2.address, ETH, 41472000)
      ).to.revertedWith('Staking: FORBIDDEN');
      await expect(
        boostedStaking
          .connect(whale2)
          .stakeLockedFor(whale2.address, owner.address, ETH, 41472000)
      ).to.revertedWith('Staking: FORBIDDEN');
    });
    ...
    it(' - Should not work lockTime = 0', async () => {
      await expect(
        boostedStaking.stakeLockedFor(whale.address, whale.address, ETH, 0)
      ).to.revertedWith('Cannot wait for a negative number');
      await expect(
        boostedStaking.stakeLockedFor(whale.address, whale2.address, ETH, 0)
      ).to.revertedWith('Cannot wait for a negative number');
      await expect(
        boostedStaking.stakeLockedFor(whale.address, owner.address, ETH, 0)
      ).to.revertedWith('Cannot wait for a negative number');
    });

    it(' - Should not work for lockTime < 7 days.', async () => {
      await expect(
        boostedStaking.stakeLockedFor(whale.address, whale.address, ETH, 604700)
      ).to.revertedWith('Minimum stake time not met (' + 604800 + ')');
      await expect(
        boostedStaking.stakeLockedFor(whale.address, owner.address, ETH, 604700)
      ).to.revertedWith('Minimum stake time not met (' + 604800 + ')');
      await expect(
        boostedStaking.stakeLockedFor(
          whale.address,
    ...
    it(' - Should not work for lockTime > 3Y', async () => {
      ...
      ).to.revertedWith('You are trying to stake for too long');
      await expect(
        boostedStaking.stakeLockedFor(
          whale.address,
          whale2.address,
    ...
    it(' - Should work for 1 account where staker is spender', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(0);
    });
    ...
    it(' - Should work for 2 accounts with same amount where staker is spender', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.mul(2));
      expect(await boostedStaking.lockedBalanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(whale2.address)).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(0);
    });
    ...
    it(' - Should work for 2 accounts with different amounts', async () => {
      ...
      await expect(
        boostedStaking
          .connect(owner)
          .stakeLockedFor(whale2.address, whale2.address, ETH.mul(2), 41472000)
      )
        .to.emit(boostedStaking, 'StakeLocked')
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.mul(3));
      expect(await boostedStaking.lockedBalanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.lockedBalanceOf(whale2.address)).to.eq(
        ETH.mul(2)
      );
      expect(await boostedStaking.lockedBalanceOf(owner.address)).to.eq(0);
    });
    ...
    it(' - Should work for 2 accounts with different amounts where another user is spender', async () => {
      ...
      await expect(
        boostedStaking
          .connect(owner)
          .stakeLockedFor(whale2.address, owner.address, ETH.mul(2), 41472000)
      )
        .to.emit(boostedStaking, 'StakeLocked')
        .withArgs(whale2.address, ETH.mul(2), 41472000);
  ...
  describe('- Withdraw', async () => {
    ...
    it(' - Should fail for amount = 0', async () => {
      await expect(boostedStaking.withdraw(0)).to.revertedWith(
        'Cannot withdraw 0'
      );
    });

    it(' - Should not withdraw for non staker', async () => {
      await expect(boostedStaking.connect(owner).withdraw(ETH)).to.revertedWith(
        ''
      );

      await expect(boostedStaking.connect(whale).withdraw(ETH)).to.revertedWith(
        ''
      );
    });
    ...
    it(' - Should not work if withdrawing > staked', async () => {
      ...
      expect(await arth.balanceOf(owner.address)).to.eq(
        ownerARTHBalanceBeforeStaking.sub(ETH)
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking.add(ETH)
      );
      expect(await boostedStaking.totalSupply()).to.eq(ETH);
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(ETH);

      await expect(
        boostedStaking.connect(owner).withdraw(ETH.mul(105).div(100))
      ).to.revertedWith('');
    ...
    it(' - Should work for 1 account', async () => {
      ...
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(0);
      expect(await boostedStaking.totalSupply()).to.eq(0);
      expect(await arth.balanceOf(owner.address)).to.eq(
        ownerARTHBalanceBeforeStaking
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking
      );
    });
    ...
    it(' - Should work properly for 1 account if withdrawing < staked', async () => {
      ...
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(HALF_ETH);
      expect(await boostedStaking.totalSupply()).to.eq(HALF_ETH);
      expect(await arth.balanceOf(owner.address)).to.eq(
        ownerARTHBalanceBeforeStaking.sub(HALF_ETH)
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking.add(HALF_ETH)
      );
    });
    ...
    it(' - Should work properly if non staker tries withdraw after someone has staked', async () => {
      ...
      await expect(
        boostedStaking.connect(whale).withdraw(HALF_ETH)
      ).to.revertedWith('');
      expect(await arth.balanceOf(owner.address)).to.eq(
        ownerARTHBalanceBeforeStaking.sub(ETH)
      );
    ...
    it(' - Should work for 2 accounts with same amount', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.mul(2));
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(ETH);
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking.add(ETH.mul(2))
      );
      ...
      expect(await boostedStaking.totalSupply()).to.eq(0);
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(0);
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(0);
      expect(await arth.balanceOf(owner.address)).to.eq(
        ownerARTHBalanceBeforeStaking
      );
      expect(await arth.balanceOf(whale.address)).to.eq(
        whaleARTHBalanceBeforeStaking
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking
      );
    ...
    it(' - Should work if someone has staked on my behalf', async () => {
      ...
      expect(await arth.balanceOf(whale.address)).to.eq(
        whaleARTHBalanceBefore.add(ETH)
      );
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(0);
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBefore
      );
    });
    ...
    it(' - Should work for 2 accounts with different amount', async () => {
      ...
      expect(await boostedStaking.totalSupply()).to.eq(ETH.mul(3));
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(ETH.mul(2));
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking.add(ETH.mul(3))
      );
      ...
      expect(await arth.balanceOf(whale.address)).to.eq(
        whaleARTHBalanceBeforeStaking.sub(ETH.mul(2))
      );
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(ETH.mul(2));
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking.add(ETH).add(ETH)
      );
  ...
  describe('- Withdraw locked', async () => {
    ...
    it(' - Should not withdraw if non staker', async () => {
      ...
      await expect(
        boostedStaking.connect(owner).withdrawLocked(kedIDSha3)
      ).to.revertedWith('Stake not found');

      await expect(
        boostedStaking.connect(whale).withdrawLocked(kedIDSha3)
      ).to.revertedWith('Stake not found');
    });
    ...
    it(' - Should work for some who locks stake on my behalf', async () => {
      ...
      await boostedStaking.stakeLockedFor(
        whale.address,
        owner.address,
        ETH,
        604800
      ...
      expect(await arth.balanceOf(whale.address)).to.eq(
        whaleARTHBalanceBeforeStaking.add(ETH)
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking
      );
    ...
    it(' - Should work for 2 account with same amount', async () => {
      ...
      expect(await arth.balanceOf(whale.address)).to.eq(
        whaleARTHBalanceBeforeStaking.sub(ETH)
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking.add(ETH).add(ETH)
      );
      ...
      const lockedStakeOwner = await boostedStaking._lockedStakesOf(
        owner.address
      );
      const lockedStakeWhale = await boostedStaking._lockedStakesOf(
        whale.address
      );
      await advanceTimeAndBlock(provider, 604800);
      await boostedStaking
        .connect(owner)
        .withdrawLocked(lockedStakeOwner[0].kekId);
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(0);
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(ETH);
      expect(await boostedStaking.totalSupply()).to.eq(ETH);
      expect(await arth.balanceOf(owner.address)).to.eq(
      ...
      await boostedStaking
        .connect(whale)
        .withdrawLocked(lockedStakeWhale[0].kekId);
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(0);
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(0);
      expect(await boostedStaking.totalSupply()).to.eq(0);
      expect(await arth.balanceOf(owner.address)).to.eq(
    ...
    it(' - Should work for 2 account with different amount', async () => {
      ...
      expect(await arth.balanceOf(whale.address)).to.eq(
        whaleARTHBalanceBeforeStaking.sub(ETH).sub(ETH)
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBeforeStaking.add(ETH).add(ETH).add(ETH)
      );
      expect(await boostedStaking.totalSupply()).to.eq(ETH.add(ETH).add(ETH));
      expect(await boostedStaking.balanceOf(owner.address)).to.eq(ETH);
      expect(await boostedStaking.balanceOf(whale.address)).to.eq(ETH.add(ETH));
  ...
  describe('- Recover token', async () => {
    it('- Should not recover staking token', async () => {
      ...
      await boostedStaking.stake(ETH);
      await expect(
        boostedStaking.recoverERC20(arth.address, ETH)
      ).to.revertedWith('');
    });
    ...
    it('- Should recover staking token', async () => {
      ...
      const contractMAHABalanceBefore = await maha.balanceOf(
        boostedStaking.address
      );
      ...
      await expect(boostedStaking.recoverERC20(maha.address, ETH))
        .to.emit(boostedStaking, 'Recovered')
        .withArgs(maha.address, ETH);

      expect(await arth.balanceOf(owner.address)).to.eq(ownerARTHBalanceBefore);
      expect(await maha.balanceOf(owner.address)).to.eq(
        ownerMAHABalanceBefore.add(ETH)
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBefore
      );
      expect(await maha.balanceOf(boostedStaking.address)).to.eq(
        contractMAHABalanceBefore.sub(ETH)
      );
    });
    ...
    it('- Should recover staking token', async () => {
      ...
      expect(await arth.balanceOf(owner.address)).to.eq(ownerARTHBalanceBefore);
      expect(await maha.balanceOf(owner.address)).to.eq(
        ownerMAHABalanceBefore.add(ETH.div(2))
      );
      expect(await arth.balanceOf(boostedStaking.address)).to.eq(
        contractARTHBalanceBefore
      );
      expect(await maha.balanceOf(boostedStaking.address)).to.eq(
        contractMAHABalanceBefore.sub(ETH.div(2))
      );
    });
  });
  ...
  describe('- Get rewards', async () => {
    ...
    describe(' - Without proper amount for people using Stake', async () => {
      it('  - Should not work if not staked', async () => {
        ...
        await expect(boostedStaking.connect(owner).getReward()).to.revertedWith(
          'BoostedStaking: rewards = 0'
        );

        expect(await maha.balanceOf(owner.address)).to.eq(
          ownerMAHABalanceBefore
        );
      });
      ...
      it('  - Should work for 1 account', async () => {
        ...
        expect(await maha.balanceOf(owner.address)).to.gt(
          ownerMAHABalanceBefore
        );
        expect(await maha.balanceOf(boostedStaking.address)).to.lt(
          contractMAHABalanceBefore
        );
      });
      ...
      it('  - Should work for 2 account, that stake with same amount and claim at same time', async () => {
        ...
        await boostedStaking.getReward();
        let newMahaBalanceOfContract = await maha.balanceOf(
          boostedStaking.address
        );
        expect(await arth.balanceOf(owner.address)).to.eq(
          ownerARTHBalanceBefore.sub(ETH)
        ...
        expect(await maha.balanceOf(whale.address)).to.eq(
          whaleMAHABalanceBefore
        );
        expect(await maha.balanceOf(boostedStaking.address)).to.lt(
          contractMAHABalanceBefore
        );
        ...
        expect(await maha.balanceOf(whale.address)).to.gt(
          whaleMAHABalanceBefore
        );
        expect(await maha.balanceOf(boostedStaking.address)).to.lt(
          contractMAHABalanceBefore
      ...
      it('  - Should work for 2 account, that stake with different amount and claim at same time', async () => {
        ...
        expect(await arth.balanceOf(boostedStaking.address)).to.lt(
          contractMAHABalanceBefore
        );
    ...
    describe(' - Without proper amount for people using Stake For', async () => {
      ...
      it('  - Should work for 2 account, that stake with same amount and claim at same time', async () => {
        ...
        expect(await maha.balanceOf(whale2.address)).to.eq(
          whale2MAHABalanceBefore
        );
        expect(await maha.balanceOf(boostedStaking.address)).to.lt(
          contractMAHABalanceBefore
        );
        ...
        expect(await maha.balanceOf(whale2.address)).to.gt(
          whale2MAHABalanceBefore
        );
        expect(await maha.balanceOf(boostedStaking.address)).to.lt(
          contractMAHABalanceBefore
      ...
      it('  - Should work for 2 account, that stake with different amount and claim at same time', async () => {
        ...
        await boostedStaking
          .connect(owner)
          .stakeFor(whale2.address, owner.address, ETH.mul(2));
        await advanceTimeAndBlock(provider, 2 * 24 * 60 * 60);
        ...
        expect(await arth.balanceOf(owner.address)).to.eq(
          ownerARTHBalanceBefore.sub(ETH).sub(ETH).sub(ETH)
        );
        expect(await arth.balanceOf(whale.address)).to.eq(
          whaleARTHBalanceBefore
        );
    ...
    describe(' - Without proper amount for people using Stake Locked For', async () => {
      ...
      it('  - Should work for 1 account', async () => {
        ...
        await boostedStaking
          .connect(owner)
          .stakeLockedFor(whale.address, owner.address, ETH, 604801);
        await advanceTimeAndBlock(provider, 2 * 24 * 60 * 60);
      ...
      it('  - Should work for 2 account, that stake with same amount and claim at same time', async () => {
        ...
        await boostedStaking
          .connect(owner)
          .stakeLockedFor(whale2.address, owner.address, ETH, 604801);
        await advanceTimeAndBlock(provider, 2 * 24 * 60 * 60);
      ...
      it('  - Should work for 2 account, that stake with different amount and claim at same time', async () => {
        ...
        await boostedStaking
          .connect(owner)
          .stakeLockedFor(whale2.address, owner.address, ETH.mul(2), 604801);
        await advanceTimeAndBlock(provider, 2 * 24 * 60 * 60);
