...
contract ArthBondIssuerWithDeposits is AccessControl {
    ...
    //     }
    // }
    // // Minimum amount of ARTH needed to buy FXB
    ...
    // }
    // function buyFXBfromAMM(uint256 arth_amount, uint256 fxb_out_min, bool should_deposit) external notBuyingPaused returns (uint256 fxb_out, uint256 fxb_fee_amt, bytes32 kek_id) {
    //     require(isInEpoch(), 'Not in an epoch');
    ...
    //     uint256 fxb_out = getAmountOutNoFee(arth_amount, ARTH.balanceOf(address(this)), FXB.balanceOf(address(this)));
    ...
    //     }
    // }
    // function sellFXBintoAMM(uint256 fxb_amount, uint256 arth_out_min, bool sell_excess_at_floor) external notSellingPaused returns (uint256 fxb_at_market_price, uint256 fxb_at_floor_price, uint256 arth_out, uint256 arth_fee_amt) {
    //     require(isInEpoch(), 'Not in an epoch');
    ...
    //     arth_out = getAmountOutNoFee(fxb_at_market_price, FXB.balanceOf(address(this)), ARTH.balanceOf(address(this)));
    //     // Add in the ARTH from the floor-priced portion
    //     uint256 the_floor_price = floor_price();
    //     arth_out = arth_out.add(fxb_at_floor_price.mul(the_floor_price).div(PRICE_PRECISION));
    ...
    // }
    // function redeemDepositedFXB(bytes32 kek_id) external notDepositRedeemingPaused returns (uint256 arth_out, uint256 arth_fee) {
    ...
    //     // Give 1 ARTH per 1 FXB, minus the redemption fee
    //     arth_fee = fxb_amount.mul(redemption_fee).div(PRICE_PRECISION);
    //     uint256 arth_out = fxb_amount.sub(arth_fee);
    //     ARTH.pool_mint(msg.sender, arth_out);
    //     emit FXB_Deposit_Redeemed(msg.sender, fxb_amount, arth_out, epoch_end, kek_id);
    // }
    ...
    // // The contraction must occur at the current AMM price, so both sides (ARTH and FXB) need to be burned
    ...
    //     ARTH.burn(fxb_contraction_amount.mul(fxb_spot_price).div(PRICE_PRECISION));
    //     // Burn the required FXB
    //     FXB.burn(fxb_contraction_amount);
    // }
    // // Allows for minting new FXB in the middle of an epoch
    // // The expansion must occur at the current AMM price, so both sides (ARTH and FXB) need to be minted
    ...
    //     ARTH.pool_mint(address(this), fxb_expansion_amount.mul(fxb_spot_price).div(PRICE_PRECISION));
    ...
    //     {
    //         uint256 desired_arth_amount = max_fxb_outstanding.mul(PRICE_PRECISION.sub(initial_discount)).div(PRICE_PRECISION);
    ...
    // event FXB_Deposit_Redeemed(address indexed from, uint256 fxb_amount, uint256 arth_out, uint256 epoch_end, bytes32 kek_id);
