...
contract FraxBond_NFT is ERC721, AccessControl {
    ...
    IFraxPartial private FRAX =
        IFraxPartial(0x853d955aCEf822Db058eb8505911ED77F175b99e);
    ...
    modifier onlyByControllerOrGovernance() {
        require(
            msg.sender == controller_address || msg.sender == timelock_address,
            "You are not the controller or the governance timelock"
        );
        _;
    }
    ...
    constructor(
        ...
        ERC721(
            ...
            FraxBond_NFT_Library.fxb_symboler(
                _series,
                _face_value,
                _maturity_months
            )
        )
    {
        ...
        super._setBaseURI(
            FraxBond_NFT_Library.concatenate3(
                "https://api.frax.finance/nft/frax-bonds/",
                FraxBond_NFT_Library.fxb_symboler(
                    _series,
                    _face_value,
                    _maturity_months
    ...
    function bondDetails(uint256 serial_number)
        external
        view
        returns (BondViewDetails memory bond_details)
    {
    ...
    function calcPenaltyMultiplier(uint256 time_elapsed)
        public
        view
        returns (uint256 penalty_multiplier)
    {
        if (time_elapsed >= maturity_secs) penalty_multiplier = 0;
            ...
            penalty_multiplier = uint256(1e6).sub(
                numerator.mul(1e6).div(denominator)
            );
        }
    }
    ...
    function issueBond(bool allow_unsafe_mint)
        external
        notMintPaused
        returns (uint256 serial_number)
    {
    ...
    function redeemBond(uint256 serial_number)
        external
        notRedeemPaused
        returns (uint256 maturity_value, uint256 return_value)
    {
        // Make sure the caller is the owner of the bond
        require(
            msg.sender == ownerOf(serial_number),
            "You are not the bond owner"
        );

        // Get the bond data and make sure it isn't already redeemed
        BondData storage thisBond = bondData[serial_number];
        require(
            thisBond.redeemed_timestamp == 0,
            "Bond has already been redeemed"
        );

        // Make sure you are not redeeming too early
        uint256 time_elapsed = (block.timestamp).sub(thisBond.issue_timestamp);
        require(
            time_elapsed >= min_early_redeem_secs,
            "You are trying to redeem too early"
        );
        ...
        maturity_value = face_value.sub(purchase_price).mul(time_elapsed).div(
            maturity_secs
        );
        ...
        maturity_value = maturity_value.sub(
            maturity_value
                .mul(penalty_multiplier)
                .mul(max_early_redemption_penalty_pct)
                .div(1e12)
        );
    ...
    function setMintLimit(uint256 _new_mint_limit)
        external
        onlyByControllerOrGovernance
    {
