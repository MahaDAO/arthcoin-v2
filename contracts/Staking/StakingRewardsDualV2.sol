    ...
    Pausable
{
    ...
    function migratorApprovedForStaker(
        ...
        if (staker_allowed_migrators[staker_address][migrator_address] == true)
            return true;
    ...
    function _stake(
        ...
        _unlocked_balances[staker_address] = _unlocked_balances[staker_address]
            .add(amount);
        _boosted_balances[staker_address] = _boosted_balances[staker_address]
            .add(amount);
    ...
    function _stakeLocked(
        ...
        // Staking token balance and boosted balance
        _locked_balances[staker_address] = _locked_balances[staker_address].add(
            amount
        );
        _boosted_balances[staker_address] = _boosted_balances[staker_address]
            .add(boostedAmount);
    ...
    function _withdrawLocked(
        ...
        if (theAmount > 0) {
            // Staking token balance and boosted balance
            _locked_balances[staker_address] = _locked_balances[staker_address]
                .sub(theAmount);
            _boosted_balances[staker_address] = _boosted_balances[
                staker_address
            ]
                .sub(boostedAmount);
        ...
        isMigrating
    {
        ...
        _withdraw(
            staker_address,
            msg.sender,
            _unlocked_balances[staker_address]
        );
    }

    // Used for migrations
    function migrator_withdraw_locked(address staker_address, bytes32 kek_id)
        external
        isMigrating
    {
        require(
