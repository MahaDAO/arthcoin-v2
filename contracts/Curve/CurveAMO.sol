...
contract CurveAMO is AccessControl {
        ...
        returns (uint256[9] memory allocations)
    {
        ...
        uint256 frax_withdrawable =
            (collat_info[0])
                .mul(FRAX.global_collateral_ratio())
                .mul(FRAX.balanceOf(frax3crv_metapool_address))
                .div(collat_info[1])
                .div(uint256(1e6));
        ...
        returns (uint256[4] memory return_arr)
    {
        ...
        if (frax3crv_supply > 0) {
            _3pool_withdrawable = lp_owned
                .mul(FRAX.global_collateral_ratio())
                .mul(three_pool_erc20.balanceOf(frax3crv_metapool_address))
                .div(frax3crv_supply)
                .div(uint256(1e6));
    ...
    function get_D() public view returns (uint256) {
        ...
        uint256[2] memory _xp =
            [
                three_pool_erc20.balanceOf(frax3crv_metapool_address),
                FRAX.balanceOf(frax3crv_metapool_address)
            ];
        ...
        )
    {
        ...
        uint256 floor_price_frax =
            uint256(1e18).mul(FRAX.global_collateral_ratio()).div(1e6);

        uint256 frax_received;
        for (uint256 i = 0; i < 256; i++) {
            // how much FRAX received for $1 worth of 3CRV
            frax_received = frax3crv_metapool.get_dy(
                1,
                0,
                one_dollar_3crv,
            ...
            } else if (frax_received + convergence_window <= floor_price_frax) {
                // if overshot
                return (frax_balance, crv3_balance, i);
            }
    ...
    function fraxBalance() public view returns (uint256) {
        if (minted_frax_historical >= burned_frax_historical)
            return minted_frax_historical.sub(burned_frax_historical);
        else return 0;
    }
    ...
    function mintRedeemPart1(uint256 frax_amount)
        public
        onlyByOwnerOrGovernance
    {
        ...
        uint256 redeem_amount_E6 =
            (frax_amount.mul(uint256(1e6).sub(redemption_fee))).div(1e6).div(
                10**missing_decimals
            );
        ...
        returns (uint256 metapool_LP_received)
    {
        ...
        require(
            fraxBalance() <= max_frax_outstanding,
            "Too much FRAX would be minted [max_frax_outstanding reached]"
        );
        ...
        metapool_LP_received = frax3crv_metapool.add_liquidity(
            [_frax_amount, threeCRV_received],
            0
        );

        // Make sure the collateral ratio did not fall too much
        uint256 current_collateral_E18 =
            (FRAX.globalCollateralValue()).mul(10**missing_decimals);
        uint256 cur_frax_supply = FRAX.totalSupply();
        uint256 new_cr =
            (current_collateral_E18.mul(PRICE_PRECISION)).div(cur_frax_supply);
        require(
            new_cr >= min_cr,
    ...
    function metapoolWithdraw(uint256 _metapool_lp_in, bool burn_the_frax)
        public
        onlyByOwnerOrGovernance
        returns (uint256 frax_received)
    {
        ...
        uint256[2] memory result_arr =
            frax3crv_metapool.remove_liquidity(_metapool_lp_in, min_amounts);
    ...
    function setMaxFraxOutstanding(uint256 _max_frax_outstanding)
        external
        onlyByOwnerOrGovernance
    {
