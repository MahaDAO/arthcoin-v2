...
contract FraxPool is AccessControl {
    ...
    function borrow(uint256 _amount) external {
        ...
        require(
            ...
            'FraxPool: Insufficent funds in the pool'
        );

        collateral_token.transfer(msg.sender, _amount);

        borrowedCollateral[msg.sender] = borrowedCollateral[msg.sender].add(
            _amount
        );
    ...
    function repay(uint256 _amount) external {
        ...
        require(
            ...
            "FraxPool: Repayer doesn't not have any debt"
        );

        collateral_token.transferFrom(msg.sender, address(this), _amount);

        borrowedCollateral[msg.sender] = borrowedCollateral[msg.sender].sub(
            _amount
        );
        ...
        notMintPaused
    {
        ...
        require(
            FRAX.global_collateral_ratio() >= COLLATERAL_RATIO_MAX,
        ...
        notMintPaused
    {
        ...
        require(
            FRAX.global_collateral_ratio() == 0,
            'Collateral ratio must be 0'
        );

        uint256 frax_amount_d18 =
            FraxPoolLibrary.calcMintAlgorithmicFRAX(
    ...
    function mintFractionalFRAX(
        ...
        uint256 collateral_amount_d18 =
            collateral_amount * (10**missing_decimals);
        FraxPoolLibrary.MintFF_Params memory input_params =
            FraxPoolLibrary.MintFF_Params(
                fxs_price,
                getCollateralPrice(),
                fxs_amount,
                collateral_amount_d18,
        ...
        (uint256 mint_amount, uint256 fxs_needed) =
            FraxPoolLibrary.calcMintFractionalFRAX(input_params);
        ...
        notRedeemPaused
    {
        ...
        uint256 collateral_needed =
            FraxPoolLibrary.calcRedeem1t1FRAX(
                getCollateralPrice(),
    ...
    function redeemFractionalFRAX(
        ...
        uint256 FRAX_amount_post_fee =
            (FRAX_amount.mul(uint256(1e6).sub(redemption_fee))).div(
                PRICE_PRECISION
            );

        uint256 fxs_dollar_value_d18 =
            FRAX_amount_post_fee.sub(
        ...
        // Need to adjust for decimals of collateral
        uint256 FRAX_amount_precision =
            FRAX_amount_post_fee.div(10**missing_decimals);
        uint256 collateral_dollar_value =
            FRAX_amount_precision.mul(global_collateral_ratio).div(
                PRICE_PRECISION
            );
        uint256 collateral_amount =
        ...
        external
    {
        ...
        FraxPoolLibrary.BuybackFXS_Params memory input_params =
            FraxPoolLibrary.BuybackFXS_Params(
        ...
        uint256 collateral_equivalent_d18 =
            (FraxPoolLibrary.calcBuyBackFXS(input_params))
