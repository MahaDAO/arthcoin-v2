...
library FraxPoolLibrary {
    ...
    function calcMint1t1FRAX(uint256 col_price, uint256 collateral_amount_d18)
        public
        pure
        returns (uint256)
    {
    ...
    function calcMintAlgorithmicFRAX(
        uint256 fxs_price_usd,
    ...
    function calcMintFractionalFRAX(MintFF_Params memory params)
        internal
        pure
    ...
    function calcRecollateralizeFRAXInner(
        ...
        uint256 effective_collateral_ratio =
            global_collat_value.mul(1e6).div(frax_total_supply); //returns it in 1e6
        uint256 recollat_possible =
            (
                global_collateral_ratio.mul(frax_total_supply).sub(
                    frax_total_supply.mul(effective_collateral_ratio)
                )
            )
                .div(1e6);
